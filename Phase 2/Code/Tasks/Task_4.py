"""
    Description: Given a visual descriptor model and a value 'k', 
    the image feature space of the given color model for each location is 
    reduced along the color model values and the corresponding term-weight 
    pairs are listed. The code is then used to extract 5 similar locations 
    given a location id for the color model inputted previously. For each 
...
while(cont.lower() == 'y'):
    ...
    # based on the location id given by user
    inner_cont = 'y'
    while(inner_cont.lower() == 'y'):
        ...
        print("\n\n Similarity score for location id: ", loc, " location name: ", location_id[loc], " Color Model: ", model)
        print("\nCosine Similarity: ")
        for row in sum_temp:
            print(row)
            print()
        
        inner_cont = input("Do you want to check for another location with same color model? (Y/N): ")
    cont = input("Do you want to continue with different color model? (Y/N): ")    