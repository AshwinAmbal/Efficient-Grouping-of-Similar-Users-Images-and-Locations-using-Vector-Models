...
Description: The code below performs K-Means Clustering with a cluster size of 10
with 10 image samples taken from each cluster and written into the edited images 
folder. These images are used for Task_4.
"""
    ...
    for mod in range(0,10):
        ...
        for i in range(0,1000):
            if len(final_list) == 100 :
                break
            if len(means[i%10]) != 0:
                final_list.append(list(df[means[i%10].pop()]))
            
        
        with open('C:\\MWDB Project\\Phase 1\\Code\\CSV\\img_edit\\{}.csv'.format(location_id[loca] + " " + models[mod] + 'Edit'), 'w', encoding = 'utf8', newline='') as outcsv:   
